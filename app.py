...
def load_models():
    ...
    for name, path, builder in [
        ("ANN", "ann_model.h5",    _build_ann),
        ("CNN", "cnn_model.keras", _build_cnn),
    ]:
        try:
            ...
            try:
                ...
                st.error(f"{name} load failed: {e2}")
    return models
    ...
    if predict_btn and canvas_result is not None:
        ...
        if img_data is not None:
            ...
            if brightness < 30:
                ...
                arr_input = arr.reshape(1, 28, 28) if model_choice == "ANN" else arr.reshape(1, 28, 28, 1)
